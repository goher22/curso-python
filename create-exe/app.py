...
def generate_exe():
    ...
    urlRest = data.get('urlRest')

    if not device or not idDevice or not urlRest:
        return 'Missing device or idDevice or urlRest', 400
...
if __name__ == "__main__":
    ...
    try:
        ...
        dist_exe_file = os.path.join('dist', 'temp_script')
        if os.name == 'nt':
            dist_exe_file += '.exe'
        
        if os.path.exists(dist_exe_file):
            return send_file(dist_exe_file, as_attachment=True)
        else:
            return f'Executable file not found at expected location: {dist_exe_file}', 500
